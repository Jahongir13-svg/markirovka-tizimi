...
// ─── STORAGE ─────────────────────────────────────────────────────────────────
const STORAGE_KEY = "marking_platform_v2";
...
// ─── TOAST ──────────────────────────────────────────────────────────────────
interface ToastData { msg: string; type: string }
...
// ─── LOGIN ──────────────────────────────────────────────────────────────────
function LoginScreen({ teams, loginTeam, setLoginTeam, loginRole, setLoginRole, adminPass, setAdminPass, onLogin, toast }: {
  teams: Team[];
  loginTeam: number;
...
// ─── WORKER SCREEN ───────────────────────────────────────────────────────────
function WorkerScreen({ team, categories, records, onAdd, onLogout, onStats, toast, showToast }: {
  team: Team;
  categories: Category[];
  records: Record[];
  onAdd: (r: Record) => void;
...
// ─── ADMIN SCREEN ────────────────────────────────────────────────────────────
function AdminScreen({ db, setDb, onLogout, onStats, toast, showToast }: {
  db: DbData;
  setDb: (d: DbData) => void;
  onLogout: () => void;
  onStats: () => void;
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 20 }}>
        ...
        <StatCard icon="👥" label="Faol komandalar" value={new Set(todayRecs.map(r => r.teamId)).size.toString()} color="#10b981" big />
      </div>
...
function AdminTeams({ db, setDb, showToast }: { db: DbData; setDb: (d: DbData) => void; showToast: (msg: string, type?: string) => void }) {
  const [newName, setNewName] = useState("");
  const [newColor, setNewColor] = useState("#6366f1");

  function addTeam() {
    if (!newName.trim()) { showToast("Nom kiriting!", "error"); return; }
    const team = { id: Date.now(), name: newName.trim(), color: newColor };
    setDb({ ...db, teams: [...db.teams, team] });
    setNewName(""); showToast("Komanda qo'shildi!");
  }

  function removeTeam(id: number) {
    setDb({ ...db, teams: db.teams.filter(t => t.id !== id) });
...
function AdminCategories({ db, setDb, showToast }: { db: DbData; setDb: (d: DbData) => void; showToast: (msg: string, type?: string) => void }) {
  const [newName, setNewName] = useState("");
  ...
  function addCat() {
    if (!newName.trim()) { showToast("Nom kiriting!", "error"); return; }
    const c = { id: Date.now().toString(), name: newName.trim(), points: Number(newPts), icon: newIcon };
    setDb({ ...db, categories: [...db.categories, c] });
    setNewName(""); showToast("Kategoriya qo'shildi!");
  }

  function removeCat(id: string) {
    setDb({ ...db, categories: db.categories.filter(c => c.id !== id) });
    showToast("O'chirildi");
  }
    ...
    <div>
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input style={{ ...S.input, width: 48 }} placeholder="🏷" value={newIcon} onChange={e => setNewIcon(e.target.value)} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {db.categories.map(c => (
            <div key={c.id} style={S.recRow}>
              ...
              <button onClick={() => removeCat(c.id)} style={{ ...S.btnSm, background: "#450a0a", color: "#fca5a5", padding: "2px 8px" }}>✕</button>
            </div>
          ))}
        </div>
      </div>
...
function AdminRecords({ db, setDb, showToast }: { db: DbData; setDb: (d: DbData) => void; showToast: (msg: string, type?: string) => void }) {
  ...
  function deleteRecord(id: number) {
    setDb({ ...db, records: db.records.filter(r => r.id !== id) });
...
function AdminExport({ db, showToast }: { db: DbData; showToast: (msg: string, type?: string) => void }) {
  ...
  function exportExcel() {
    ...
    if (period === "week") startDate = startOfWeek(now.toISOString());
      ...
      <div style={S.field}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[["week", "📅 Haftalik"], ["month", "📆 Oylik"], ["all", "🗂 Hammasi"]].map(([v, l]) => (
            <button key={v} onClick={() => setPeriod(v as string)} style={{
              ...S.btn, flex: 1,
...
// ─── STATS SCREEN ─────────────────────────────────────────────────────────────
function StatsScreen({ db, onBack, currentUser, showToast, toast }: {
  db: DbData;
  onBack: () => void;
  currentUser: { role: string; teamId?: number } | null;
  showToast: (msg: string, type?: string) => void;
  toast: ToastData | null;
}) {
  ...
  function getFiltered() {
    ...
    else if (period === "week") startDate = startOfWeek(now.toISOString());
    else startDate = startOfMonth(now.toISOString());
...
// ─── SHARED COMPONENTS ───────────────────────────────────────────────────────
function StatCard({ icon, label, value, color, big }: { icon: string; label: string; value: string; color: string; big?: boolean }) {
  return (
    <div style={{
