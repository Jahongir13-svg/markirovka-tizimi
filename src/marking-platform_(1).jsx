...
// ─── CIRCLE KPI ─────────────────────────────────────────────────────────────
function CircleKPI({ value, target, label, color }) {
...
export default function App() {
  const [view, setView] = useState("login"); // login | admin | worker | stats
  const [currentUser, setCurrentUser] = useState(null); // { role:'admin'|'worker', teamId }
  ...
  if (view === "worker") return <WorkerScreen
...
function WorkerScreen({ team, categories, records, onAdd, onLogout, onStats, toast, showToast }) {
  ...
  // TCD scanner: barcodes come fast, ending with Enter
  useEffect(() => {
    function onKey(e) {
      if (e.key === "Enter") {
        if (scanBuffer.length > 3) {
          // treat barcode scan as qty=1
          handleSubmit(1, catId, scanBuffer);
          setScanBuffer("");
        }
        return;
      }
  ...
  function handleSubmit(qtyVal, category, barcode = null) {
    ...
    setQty("");
    showToast(barcode
      ? `✅ Scanner: ${n} ta ${c?.name} qo'shildi`
      : `✅ ${n} ta ${c?.name} qo'shildi (+${rec.points} ball)`
    );
  }
    ...
    <div style={S.page}>
      ...
      <div style={{ maxWidth: 540, margin: "0 auto", padding: "16px 12px" }}>
        ...
        <div style={{ ...S.card, marginBottom: 16 }}>
          ...
          <div style={S.field}>
            <label style={S.label}>Son kiriting (yoki scanner bilan o'qing)</label>
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                onKeyDown={e => e.key === "Enter" && handleSubmit(qty, catId)}
                autoFocus
              />
              <button onClick={() => handleSubmit(qty, catId)} style={{ ...S.btn, ...S.btnPrimary, padding: "0 20px", fontSize: 18 }}>
                ✓
              </button>
            </div>
          </div>
...
function AdminScreen({ db, setDb, onLogout, onStats, toast, showToast }) {
  const [tab, setTab] = useState("dashboard"); // dashboard | teams | categories | records
...
function AdminTeams({ db, setDb, showToast }) {
...
function AdminCategories({ db, setDb, showToast }) {
  const [newName, setNewName] = useState("");
...
function AdminRecords({ db, setDb, showToast }) {
        ...
        <div style={{ maxHeight: 450, overflowY: "auto" }}>
          ...
          {filtered.map(r => (
            <div key={r.id} style={{ ...S.recRow, marginBottom: 6 }}>
              ...
              <button onClick={() => deleteRecord(r.id)} style={{ ...S.btnSm, background: "#450a0a", color: "#fca5a5", padding: "2px 6px" }}>✕</button>
            </div>
          ))}
        </div>
      </div>
...
function AdminExport({ db, showToast }) {
